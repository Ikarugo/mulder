...
The catalog reports each triage root as a `triage_collection` (one system, named after the host). The extraction planner then runs the Windows artifact parsers (registry, Prefetch, Amcache, ShimCache, `$MFT`, the USN journal, LNK files, Jump Lists, Shellbags, SRUM, EVTX, Hayabusa, Chainsaw, YARA) with the triage root as `image_path`; wherever those tools would read a disk image through Sleuth Kit or a FUSE mount, they read the collected files directly. Tools that only make sense on a raw image (`run_fls`, `run_mmls`, carving, `detect_masquerading`, shadow-copy and BitLocker tools) return `not_applicable_triage` with a pointer to the tools that apply. Scripts (`.ps1`, `.bat`, `.vbs`, `.js`, `.hta`...), archives and SQLite databases are catalogued when they sit where users write (`Users`, `ProgramData`, `Windows\Temp`, `$Recycle.Bin`, `PerfLogs`, the volume root), leaving out program trees such as `node_modules` or `AppData\Local\Programs`; the thousands shipped under `Windows` and `Program Files` are not listed. PowerShell `ConsoleHost_history.txt` files are always catalogued. When a collection was prepared with `prepare-triage`, the scan reuses the SHA-256 hashes from its manifest instead of reading every file again. SQLite databases found as files (browser history, Windows Timeline, chat applications) are read with `query_sqlite_file`, which lists the schema when called with an empty query. `parse_cryptnet_url_cache` recovers files downloaded with `certutil -urlcache` (URL, time, hash of the cached copy), and `extract_mft_record` reads small files, such as scripts and `.bat` droppers, from their `$MFT` record even when they were deleted or not collected. ESE databases are read in Python (dissect.esedb), so they work in the Linux container and on copies taken from a running system: `run_srum_parser` reads SRUM, `parse_windows_search` the Windows Search index (files, e-mail headers, contacts), and `query_ese_database` any other `.edb` or `WebCacheV01.dat`. `parse_scheduled_tasks` reads every scheduled task from both its XML and the SOFTWARE hive's TaskCache, so the exact command line, the UTC registration and last-run times, and tasks hidden from `schtasks` or edited in the registry only are all in `tasks.scheduled`.
